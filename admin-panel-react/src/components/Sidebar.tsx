...
export function Sidebar({ isOpen, onClose, activePage, onNavigate }: SidebarProps) {
  ...
  const navItems = useMemo(() => [
    ...
    { icon: Calendar, label: t("navigation.appointments"), page: "appointments", badge: (stats.totalAppointments || 0).toString() },
    { icon: Wrench, label: t("navigation.services"), page: "services", badge: (stats.totalServices || 0).toString() },
    { icon: BarChart3, label: t("navigation.analytics"), page: "analytics" },
    { icon: Building2, label: t("navigation.organizations"), page: "organizations" },
    { icon: Bot, label: t("navigation.botManagement"), page: "bot-management", badge: t("navigation.badges.new"), badgeVariant: "secondary" as const },
    { icon: Sparkles, label: t("navigation.aiAssistant"), page: "ai", badge: t("navigation.badges.beta"), badgeVariant: "secondary" as const },
    { icon: Settings, label: t("navigation.settings"), page: "settings" },
  ], [t, stats.totalAppointments, stats.totalServices]);
          ...
          <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 flex-shrink-0">
            ...
            <div className="px-2 sm:px-3 py-2 sm:py-2.5 bg-white/10 dark:bg-white/5 rounded-lg backdrop-blur-sm">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                ...
                <p className="text-[10px] sm:text-xs font-medium">{t("navigation.adminStatus")}</p>
              </div>
              <div className="flex items-center gap-1.5 sm:gap-2">
                {isAdminLinked ? (
                  <>
                    <CheckCircle2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-300 dark:text-emerald-400 flex-shrink-0" />
                    <span className="text-[10px] sm:text-xs text-white/90 dark:text-white/80">{t("navigation.telegramLinked")}</span>
                  </>
                ) : (
                  <>
                    <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border-2 border-amber-300 dark:border-amber-400 flex-shrink-0" />
                    <span className="text-[10px] sm:text-xs text-amber-200 dark:text-amber-300">{t("navigation.notLinked")}</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-1.5 sm:py-2 bg-white/10 dark:bg-white/5 rounded-lg backdrop-blur-sm">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] sm:text-xs text-white/70 dark:text-white/60">{t("navigation.today")}</p>
                <p className="text-xs sm:text-sm font-medium truncate">
                  {isLoading ? "..." : `${stats.todayAppointments || 0} ${t("navigation.appointmentsShort")}`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-1.5 sm:py-2 bg-white/10 dark:bg-white/5 rounded-lg backdrop-blur-sm">
              <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-[10px] sm:text-xs text-white/70 dark:text-white/60">{t("navigation.thisWeek")}</p>
                <p className="text-xs sm:text-sm font-medium truncate">
                  {isLoading ? "..." : `${stats.weekAppointments || 0} ${t("navigation.appointmentsShort")}`}
                </p>
              </div>
            </div>
